...
with open("./../results/tiempo_DP.dat") as catalog:
    t_DP = []
    for line in catalog:
        column = line.split()
...
with open("./../results/tiempo_RK.dat") as catalog:
    t_RK = []
    for line in catalog:
        column = line.split()
        if not line.startswith('#'): #skipping column labels
...
with open("./../results/energias_RK.dat") as catalog:
    E_RK = []
    for line in catalog:
        column = line.split()
        if not line.startswith('#'): #skipping column labels
...
with open("./../results/energias_DP.dat") as catalog:
    E_DP = []
    for line in catalog:
        column = line.split()
        if not line.startswith('#'): #skipping column labels
...
plt.savefig('./../plots/energias.png')
plt.show()
